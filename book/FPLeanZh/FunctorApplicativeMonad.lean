...
-/
函子 (Functor) 和 单子 (Monad) 都描述了那些仍在等待类型参数的类型的操作。
一种理解它们的方式是，`Functor` 描述了容器，其中容器内的数据可以被转换，而 `Monad` 描述了具有副作用的程序编码。
然而，这种理解是不完整的。
毕竟，`Option` 同时拥有 `Functor` 和 `Monad` 的实例，并且同时代表着一个可选值 __和__ 一个可能无法返回值的计算。
...
此外，还有一个额外的中间抽象，被称为 __应用函子 (Applicative Functors)__，它有足够的能力来编写许多有趣的程序，而且还适用于那些无法使用 `Monad` 接口的库。
