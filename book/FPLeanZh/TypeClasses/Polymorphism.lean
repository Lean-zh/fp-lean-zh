...
检查接受隐式参数或使用类型类的函数的类型需要使用一些额外的语法。
简单地写
...
这是因为 Lean 会尽力发现隐式参数，而元变量的存在表明它尚未发现足够的类型信息来这样做。
要理解函数的签名，可以使用函数名前的 at 符号 ({anchorTerm printlnNoMetas}`@`) 来抑制此功能：
...
在此输出中，实例本身已被命名为 {lit}`inst`。
此外，{moduleTerm}`Type` 后面有一个 {lit}`u_1`，它使用了 Lean 中尚未引入的功能。
暂时忽略 {moduleTerm}`Type` 的这些参数。
...
一个对列表中所有条目求和的函数需要两个实例：{moduleName}`Add` 允许将条目相加，而 {anchorTerm ListSum}`0` 的 {moduleName}`OfNat` 实例为返回空列表提供了一个合理的值：
...
:::
方括号中所需实例的规范称为__实例隐式__。
在幕后，每个类型类都定义一个结构，该结构为每个重载操作都有一个字段。
实例是该结构类型的值，每个字段都包含一个实现。
在调用站点，Lean 负责为每个实例隐式参数查找要传递的实例值。
普通隐式参数和实例隐式之间最重要的区别是 Lean 用于查找参数值的策略。
对于普通隐式参数，Lean 使用一种称为__统一__的技术来查找允许程序通过类型检查器的单个唯一参数值。
此过程仅依赖于函数定义和调用站点中涉及的特定类型。
对于实例隐式，Lean 会查阅内置的实例值表。
...
以这种方式构造的实例值是类型类的结构类型的值。
成功的递归实例搜索会产生一个结构值，该结构值引用另一个结构值。
{moduleTerm}`Add (PPoint Nat)` 的实例包含对找到的 {moduleTerm}`Add Nat` 实例的引用。
 ...
 * 一个结构类型，用于包含每个重载操作的实现
 * 一个与类同名的命名空间
 * 对于每个方法，类命名空间中的一个函数，用于从实例中检索其实现
...
主要区别在于，结构的访问器将结构值作为显式参数，而类型类方法将实例值作为实例隐式，由 Lean 自动查找。
...
为了让 Lean 找到一个实例，它的参数必须是可用的。
这意味着类型类的每个参数都必须是出现在实例之前的方法的参数。
...
这意味着 Lean 在尝试找出隐式参数 {anchorName ofNatType}`n` 时将没有任何信息可供使用。
结果将是一个非常不方便的 API。
