...
从 Python、Haskell 到 JavaScript 等各种语言都提供__读取-求值-打印-循环（REPL）__，
也称为交互式顶层环境或浏览器控制台，可以在其中输入表达式或语句。然后，该语言会计算并显示用户输入的结果。另一方面，Lean 将这些特性集成到了与编辑器的交互中，它提供的命令能让文本编辑器将程序的反馈集成到程序文本中。本章简要介绍了在编辑器中与 Lean 的交互，而 {ref "hello-world.md"}[Hello, World!] 则描述了如何在批处理模式下以传统的命令行方式使用 Lean。
