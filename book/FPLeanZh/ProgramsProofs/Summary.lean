...
但 Lean 还允许使用数学证明来完全确保函数的两个版本对 __所有__ 可能的输入返回相同的结果。
...
类型 {anchorTerm names}`Fin n` 表示严格小于 {anchorName names}`n` 的自然数。{anchorName names}`Fin` 是「有限」的缩写。
与子类型一样，{anchorTerm names}`Fin n` 是一个包含 {anchorName names}`Nat` 和证明这个 {anchorName names}`Nat` 小于 {anchorName names}`n` 的结构体。
...
Lean 为 {anchorName names}`Fin` 提供了大多数有用的数字类型类的实例。{anchorName names}`Fin` 的 {anchorName names}`OfNat` 实例会执行模运算，
而非在提供的数字大于 {anchorName names}`Fin` 可接受的数字时在编译时失败。
...
-/
{anchorTerm names}`sorry` 策略使 Lean 临时接受一个陈述，就好像它是真正的证明一样。
它可以看作类似于在 C# 中抛出 {CSharp}`NotImplementedException` 的桩（stub）方法。
任何依赖于 {anchorTerm names}`sorry` 的证明都会在 Lean 中包含一个警告。
...
-/
小心！{anchorTerm names}`sorry` 策略可以证明 __任何__ 陈述，甚至是错误的陈述。
证明 {anchorTerm names}`3 < 2` 可能会导致数组越界访问持续到运行时，意外地使程序崩溃。
在开发过程中使用 {anchorTerm names}`sorry` 很方便，但将其保留在代码中很危险。
