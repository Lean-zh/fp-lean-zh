...
命令式语言通常有两种条件：根据布尔值确定要执行哪些指令的条件 __语句（Statement）__，
以及根据布尔值确定要计算两个表达式中哪一个的条件 __表达式（Expression）__。
