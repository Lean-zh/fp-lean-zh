...
*
  * {anchorName all}`Nat`
  * 一元类型，每个 {anchorTerm all}`Nat.succ` 都有一个指针
  * 高效的任意精度整数

*
  * {anchorName all}`Int`
  * 和类型，带有表示正负值的构造子，每个包含一个 {anchorName all}`Nat`
  * 高效的任意精度整数

*
  * {anchorTerm all}`BitVec w`
  * 带有合适边界 $`2^w` 的 {anchorName all}`Fin`
  * 高效的任意精度整数
...
*
  * {anchorName all}`Char`
  * {anchorName all}`UInt32` 以及与之配对的它是有效码位的证明
  * 一般字符
...
*
  ...
  * 指向 {anchorName sequences}`α` 值的指针打包的数组
...
-/
{ref "positive-numbers"}[{anchorName Pos (module := Examples.Classes)}`Pos` 的定义] 并没有利用 Lean 将 {anchorName all}`Nat` 编译成高效类型的优势。
在运行时，它本质上是一个链表。或者，可以定义一个子类型，允许在内部使用 Lean 的快速 {anchorName all}`Nat` 类型，
如 {ref "subtypes"}[子类型的开头部分] 中所述。
在运行时，证明将被擦除。由于结果结构体只有一个数据字段，因此它会表示为该字段，
这意味着 {anchorName Pos (module := Examples.Classes)}`Pos` 的这种新表示与 {anchorName all}`Nat` 的表示相同。
...
-/
在证明定理 {anchorTerm all}`∀ {n k : Nat}, n ≠ 0 → k ≠ 0 → n + k ≠ 0` 之后，为 {anchorName Pos (module := Examples.Classes)}`Pos` 这种新的表示定义
{anchorName all}`ToString` 和 {anchorName all}`Add` 的实例。然后，定义 {anchorName all}`Mul` 的实例，在此过程中证明任何必要的定理。
