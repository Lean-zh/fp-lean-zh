...
namespace FPLeanZh -- Added this namespace declaration

register_option linter.typography.quotes : Bool := {
  defValue := true
  descr := "if true, generate curly quote suggestions" /- 如果为真，生成弯引号建议 -/
}

register_option linter.typography.dashes : Bool := {
  defValue := true
  descr := "if true, generate em and en dash suggestions" /- 如果为真，生成长破折号和短破折号建议 -/
}
  ...
  run := withSetOptionIn fun stx => do
    ...
    let suggest (what : String) (replacement : String) (linter : Lean.Option Bool) (pos : String.Pos) (stop : String.Pos := text.source.next pos) := do
      let strLit :=
        ...
        let h ← liftTermElabM <| MessageData.hint m!"Replace with Unicode" /- 替换为 Unicode -/ #[{suggestion := replacement}] (ref? := strLit)
        logLint linter strLit (m!"Use {what} ('{replacement}')" /- 使用 {what} ('{replacement}') -/ ++ h)
...
' =>
            ...
            | .atBeginning _, '"' =>
              state := .none
              suggest "curly quotes" /- 弯引号 -/ "“" linter.typography.quotes here
            | _, '"' =>
              state := .none
...
', '.', ',', ':', ';', '?', '!', '}', ')', ']'] then
            ...
            | _, '-' =>
              ...
              if dashCount == 3 then
                ...
                suggest "an em dash" /- 长破折号 -/ "—" linter.typography.dashes start dashesSpaces.stopPos
              else if dashCount == 2 then
                match state with
                | .afterDigit =>
                  suggest "an en dash" /- 短破折号 -/ "–" linter.typography.dashes here dashesSpaces.stopPos
                | .atBeginning p =>
                  suggest "an em dash" /- 长破折号 -/ "—" linter.typography.dashes p dashesSpaces.stopPos
                | .none =>
                  suggest "an em dash" /- 长破折号 -/ "—" linter.typography.dashes here dashesSpaces.stopPos
...
end FPLeanZh -- Added this namespace declaration