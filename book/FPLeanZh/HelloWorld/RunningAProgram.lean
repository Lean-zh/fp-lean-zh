...
在执行期间，该程序可能具有副作用。
这些程序被称为 {moduleTerm}`IO` __动作__。
Lean 区分表达式的__求值__（严格遵循变量值替换和无副作用子表达式约简的数学模型）和 {anchorTerm sig}`IO` 动作的__执行__（依赖外部系统与世界交互）。
...
在这种情况下，从标准输入读取一行文本__是__一个纯函数，因为每次都提供一个不同的世界作为参数。
...
该块包含一系列__语句__，这些语句既可以是局部变量（使用 {kw}`let` 引入），也可以是要执行的动作。
