 ...
 * 重写 {anchorName plusR_succ_left (module:=Examples.DependentTypes.Pitfalls)}`plusR_succ_left` 的证明，使用 {kw}`<;>` 在一行中完成。
 * 使用列表归纳证明列表连接是结合的：
   ```anchorTerm ex
   theorem List.append_assoc (xs ys zs : List α) :
       xs ++ (ys ++ zs) = (xs ++ ys) ++ zs
   ```information about the proof as the cursor is moved through it and there are a number of interactive features that make proving easier.
...
实际上，与 Lean 互动的过程要愉快得多。
Lean 在光标移动时提供有关证明的信息，并且有许多互动功能使证明更容易。
请查阅您的 Lean 开发环境的文档以获取更多信息。

本书中的方法侧重于逐步构建证明并显示产生的消息，这展示了 Lean 在编写证明时提供的各种互动反馈，尽管这比专家使用的过程慢得多。
同时，看到不完整的证明逐步趋向完整是一种对证明有益的视角。
随着您编写证明技能的提高，Lean 的反馈将不再感觉像错误，而更像是对您自己思维过程的支持。
学习互动方法非常重要。
...
代替使用诸如 {anchorName plusR_succ_left (module:=Examples.DependentTypes.Pitfalls)}`congrArg` 之类的函数和运算符，存在允许使用等式证明转换证明目标的策略。
...
{anchorName plusR_zero_left_golf_1}`plusR_zero_left` 的归纳步骤可以使用简化策略 {kw}`simp` 证明。
单独使用 {kw}`simp` 并没有帮助：
...
然而，{kw}`simp` 可以配置为使用一组定义。
就像 {kw}`rw` 一样，这些参数在列表中提供。
要求 {kw}`simp` 考虑 {anchorName plusR_zero_left_golf_1}`Nat.plusR` 的定义导致一个更简单的目标：
...
然而，使用 {kw}`exact` 有点脆弱。重命名归纳假设（在"打高尔夫"证明时可能会发生）会导致此证明停止工作。假设策略 {kw}`assumption` 解决了当前目标，如果 __任何__ 假设与之匹配：
...
-- It complains if any cases are missing, and it shows the structure of the induction clearly. But shortening proofs can often require a more liberal approach.

这个证明并不比使用展开和显式重写的先前证明短。
然而，一系列变换可以使它更短，利用 {kw}`simp` 可以解决许多类型的目标这一事实。
第一步是去掉 {kw}`induction` 末尾的 {kw}`with`。
对于结构化、可读的证明，{kw}`with` 语法是方便的。
如果缺少任何情况，它会抱怨，并且它清楚地显示归纳的结构。但是缩短证明通常需要更宽松的方法。
...
使用不带 {kw}`with` 的 {kw}`induction` 仅会产生两个目标。 {kw}`case` 策略可以像在 {kw}`induction`{lit}` ...`{kw}`with` 策略的分支中一样选择其中一个目标。换句话说，以下证明等同于前一个证明：
...
{lit}`T1 `{kw}`<;>`{lit}` T2` 将 {lit}`T1` 应用于当前目标，然后在 {lit}`T1` 创建的 _所有_ 目标中应用 {lit}`T2`。
...
因为 {kw}`simp` 既可以完成基本情况的证明，又可以在归纳步骤的证明中取得进展，所以使用它与 {kw}`induction` 和 {kw}`<;>` 缩短了证明：
...
对于初学者来说，这个证明并不容易阅读。
然而，专家用户的常见模式是使用像 {kw}`simp` 这样的强大策略处理一些简单情况，使他们可以将证明的文本集中在有趣的情况下。
...
这可以通过使用带有展开 {anchorName mirror_count_1}`BinTree.mirror` 指令的 {kw}`simp` 表达：
...
{kw}`simp` 策略可以使用附加的算术恒等式（当传递 {anchorTerm mirror_count_4}`+arith` 选项时）。
足以证明这个目标，产生：
...
{kw}`simp` 或 {kw}`simp +arith` 的参数 {lit}`*` 指示它们在简化或解决目标时使用 _所有_ 假设。
换句话说，证明也可以这样写：
