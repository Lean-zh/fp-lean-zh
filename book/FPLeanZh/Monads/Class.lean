...
:::paragraph
/--
The `Monad` instances for `Option` and `Except ε` can be created by adapting the definitions of their respective `andThen` operations:
-/
...
-/
给定慢速哺乳动物和快速鸟类的示例列表，{anchorName firstThirdFifthSeventhMonad}`firstThirdFifthSeventh` 的这个实现可以与 {moduleName}`Option` 一起使用：
...
在将 {anchorName getOrExcept}`Except` 的查找函数 {lit}`get` 重命名为更具体的名称之后，{anchorName firstThirdFifthSeventhMonad}`firstThirdFifthSeventh` 的相同实现也可以与 {anchorName getOrExcept}`Except` 一起使用：
...
{anchorName firstThirdFifthSeventhMonad}`m` 必须有 {anchorName firstThirdFifthSeventhMonad}`Monad` 实例，这一事实意味着可以使用 {lit}`>>=` 和 {anchorName firstThirdFifthSeventhMonad}`pure` 运算符。
:::
...
由于许多不同类型都是单子，因此对 __任何__ 单子多态的函数非常强大。
...
由于 {anchorName mapM}`f` 的类型直接决定了可用的效应（Effects），因此 API 设计人员可以对其进行严格控制。
:::
...
如{ref "numbering-tree-nodes"}[本章简介]所介绍的，{anchorTerm StateEx}`State σ α` 表示使用类型为 {anchorName StateEx}`σ` 的可变变量，并返回类型为 {anchorName StateEx}`α` 的值的程序。
...
可以使用 {anchorName MonadWriter}`WithLog` 表示{ref "logging"}[日志记录效应]。
...
有时 API 会使用单子来提高灵活性，但 API 的使用方可能不需要任何效应。
{deftech}__恒等单子__（Identity Monad）是一个没有任何效应的单子，允许将纯（pure）代码与 monadic API 一起使用：
...
{anchorName IdMonad}`pure` 的类型应为 {anchorTerm IdMore}`α → Id α`，但 {anchorTerm IdMore}`Id α` __归约__ 为 {anchorTerm IdMore}`α`。
类似地，{anchorName IdMonad}`bind` 的类型应为 {anchorTerm IdMore}`α → (α → Id β) → Id β`。
由于这 __归约__ 为 {anchorTerm IdMore}`α → (α → β) → β`，因此可以将第二个参数应用于第一个参数得到结果。
...
使用恒等单子时，{anchorName mapMId}`mapM` 等同于 {anchorName Names (show:=map)}`Functor.map`。
但是要以这种方式调用它，Lean 需要额外的提示来表明目标单子是 {anchorName mapMId}`Id`：
```anchor mapMId
...
导致错误的原因是：一个元变量应用于另一个元变量，使得 Lean 不会反向运行类型计算。
