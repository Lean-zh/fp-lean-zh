...
这意味着在 {lit}`Greetings/Basic.lean` 中定义的所有内容也可用于导入 {lit}`Greetings.lean` 的文件。
...
因为 {lit}`Main.lean` 导入 {lit}`Greetings.lean` 而 {lit}`Greetings.lean` 导入 {lit}`Greetings/Basic.lean`，
...
{lit}`lakefile.toml` 描述了一个__包__，它是用于分发的 Lean 代码的有条理集合，类似于 {lit}`npm` 或 {lit}`nuget` 包或 Rust crate。
包可以包含任意数量的库或可执行文件。
...
这个初始的 Lake 配置包含三个项目：
 * 位于文件顶部的__包__设置，
 * 名为 {lit}`Greeting` 的__库__声明，以及
 * 名为 {lit}`greeting` 的__可执行文件__。
...
不过，一般来说，默认值是合理的。
用 Lean 格式编写的 Lake 配置文件还可以包含__外部库__，这些是非 Lean 编写的库，要与生成的可执行文件静态链接；
__自定义目标__，这些是不适合库/可执行文件分类的构建目标；
以及__脚本__，它们本质上是 {moduleName}`IO` 动作（类似于 {moduleName}`main`），但另外还可以访问有关包配置的元数据。
...
Lean 库由分层组织的源文件集合组成，可以从中导入名称，称为__模块__。
...
名称空间也可以__选择性地__打开，只让其中一些名称无需显式前缀即可使用。
这是通过将所需的名称写在括号中来完成的。
