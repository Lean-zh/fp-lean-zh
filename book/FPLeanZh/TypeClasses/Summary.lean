...
__类型类__具有名称、参数和由多个带类型的名称组成的主体。
名称是引用重载操作的一种方式，参数确定可以重载定义的哪些方面，主体提供可重载操作的名称和类型签名。
每个可重载的操作都称为类型类的 {deftech}__方法__。
...
类型类的 {deftech}__实例__为给定参数提供方法的实现。
...
类型类参数要么是 {deftech}__输入参数__（默认），要么是 {deftech}__输出参数__（由 {moduleName}`outParam` 修饰符指示）。
...
实例可以用 {moduleTerm}`@[default_instance]` 属性标记。
