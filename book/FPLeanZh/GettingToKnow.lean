...
compiling and running a program that displays {moduleTerm}`"Hello, world!"` on the
...
本章简要介绍了在编辑器中与 Lean 的交互，而 {ref "hello-world.md"}[Hello, World!] 则描述了如何在批处理模式下以传统的命令行方式使用 Lean。
