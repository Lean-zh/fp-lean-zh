...
-/
在许多语言中，内置数据类型有特殊的优待。
例如，在 C 和 Java 中，{lit}`+` 可以被用于 {c}`float` 和 {c}`int`，但不能用于其他第三方库的数字。
类似地，数字字面量可以被直接用于内置类型，但是不能用于用户定义的数字类型。
其他语言为运算符提供 {deftech}__重载（overloading）__ 机制，使得同一个运算符可以在新的类型有意义。
...
这种限制的另一个后果是一些运算符（例如 Java 中的等号）对 __每一个__ 参数组合都要有定义，即使这样做是完全没必要的。
...
-/
Lean 用 {deftech}__类型类（type classes）__ 机制（源于 Haskell）来实现重载。
这使得运算符，函数和字面量重载与多态有一个很好的配合。
一个类型类描述了一族可重载的运算符。
要将这些运算符重载到新的类型上，你需要创建一个包含对新类型的每一个运算的实现方式的 __实例（instance）__ 。
...
-/
__类__ 和 __实例__ 这两个词可能会使面向对象程序员感到混淆，因为 Lean 中的它们与面向对象语言中的类和实例关系不大。
...
-/
Lean 的类型类更像是 Java 或 C# 中的 __接口（interface）__。
