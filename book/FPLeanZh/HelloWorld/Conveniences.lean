...
Lean 从嵌套表达式上下文中提升的 {moduleName (module := Examples.Cat)}`IO` 动作称为__嵌套动作__。
:::
...
换句话说，对于 {moduleName (module := Examples.HelloWorld)}`IO` 动作，{moduleTerm (module := Examples.HelloWorld)}`#eval` 既__求值__提供的表达式又__执行__结果动作值。
