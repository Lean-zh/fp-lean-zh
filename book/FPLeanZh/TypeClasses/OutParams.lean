...
然而，在许多情况下，更灵活并允许__异构__运算符重载可能很有用，其中参数可以具有不同的类型。
...
如 {ref "overloaded-addition"}[重载加法] 部分所述，Lean 提供了一个名为 {moduleName}`HAdd` 的类型类，用于异构地重载加法。
{moduleName}`HAdd` 类接受三个类型参数：两个参数类型和返回类型。
{moduleTerm}`HAdd Nat Pos Pos` 和 {moduleTerm}`HAdd Pos Nat Pos` 的实例允许使用普通的加法表示法来混合类型：
...
:::paragraph
{moduleName}`HAdd` 类型类的定义与以下 {moduleName}`HPlus` 的定义及其相应实例非常相似：
...
然而，{moduleName}`HPlus` 的实例远不如 {moduleName}`HAdd` 的实例有用。
...
-- Because the third type parameter for {moduleName}`HPlus` was unknown, Lean\'t carry out type class instance search, but instance search is the only way that Lean could determine the expression\'s type.
...
在这种情况下，它无法做到。
因为 {moduleName}`HPlus` 的第三个类型参数是未知的，所以 Lean 无法执行类型类实例搜索，但实例搜索是 Lean 确定表达式类型的唯一方法。
也就是说，只有当表达式应具有类型 {moduleName}`Pos` 时，{moduleTerm}`HPlus Pos Nat Pos` 实例才能应用，但程序中除了实例本身之外没有任何东西表明它应具有此类型。
...
这个问题也可以通过将 {anchorName HPlus}`γ` 声明为__输出参数__来解决。
...
这有点像 Python 或 Kotlin 中可选函数参数的默认值，只是正在选择默认__类型__。
...
__默认实例__是即使并非所有输入都已知也适用于实例搜索的实例。
当可以使用这些实例之一时，就会使用它。
...
默认实例也可以被分配__优先级__，这会影响在多个实例可能适用的情况下选择哪个实例。
