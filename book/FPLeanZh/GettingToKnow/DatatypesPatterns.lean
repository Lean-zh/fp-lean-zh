...
将值集合组合在一起的类型（如结构）称为 __积类型（Product Types）__。
但是，许多域概念不能自然地表示为结构。
...
允许选择的数据类型称为 __和类型（Sum Types）__，可以包含自身实例的数据类型称为 __递归数据类型（Recursive Datatypes）__。
递归和类型称为 __归纳数据类型（Inductive Datatypes）__，因为可以使用数学归纳法来证明关于它们的陈述。
...
__后继（Successor）__ 意味着"比...大一"，所以五的后继是六，32,185的后继是32,186。
...
在 Lean 中，这两个目的同时由 __模式匹配（Pattern Matching）__ 完成。
...
-- If {anchorName isZero}`n` 由 {anchorName isZero}`Nat.zero` 构造，则采用模式匹配的第一个分支，结果为 {anchorName isZero}`true`。
-- If {anchorName isZero}`n` 由 {anchorName isZero}`Nat.succ` 构造，则采用第二个分支，结果为 {anchorName isZero}`false`。
...
引用正在定义的名称的定义称为 __递归定义（Recursive Definitions）__。
...
像这样的代码的非递归分支称为 __基本情况（Base Cases）__。
奇数的后继是偶数，偶数的后继是奇数。
...
这种模式称为 __结构递归（Structural Recursion）__。
...
-- Similarly, multiplication $`n × k` adds $`n` to itself $`k` 次 and subtraction $`n - k` takes $`n`'s predecessor $`k` times.
