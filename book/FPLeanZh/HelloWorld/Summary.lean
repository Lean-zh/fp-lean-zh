...
相反，Lean 有一个名为 {moduleName}`IO` 的类型，它表示使用副作用的程序的__描述__。
这些描述然后由语言的运行时系统执行，该系统调用 Lean 表达式求值器来执行特定计算。
{moduleTerm}`IO α` 类型的值被称为 __{moduleName}`IO` 动作__。
...
在 {kw}`do` 表达式内部，__嵌套动作__是括号下紧跟左箭头的表达式。
...
Lean 项目组织成__包__，这些包是库和可执行文件的集合，以及有关依赖项和构建配置的信息。
