...
-/
__依值类型（Dependent types）__ 是包含非类型表达式的类型。
依值类型的一个常见来源是函数的命名参数。
例如，函数 {anchorName natOrStringThree}`natOrStringThree` 返回自然数或字符串，具体取决于传入的 {anchorName natOrStringThree}`Bool` 值：
...
-/
更多依值类型的例子包括：
  * {ref "getting-to-know/polymorphism.md"}[多态的介绍性段落] 中的 {anchorName posOrNegThree (module:= Examples.Intro)}`posOrNegThree`，其中函数的返回类型取决于参数的值。
  * {ref "type-classes/pos.md#literal-numbers"}[{anchorName OfNat (module := Examples.Classes)}`OfNat` 类型类] 中取决于具体的自然数字面量。
  * {ref "functor-applicative-monad/applicative.md#validated-input"}[{anchorName CheckedInput (module := Examples.FunctorApplicativeMonad)}`CheckedInput` 结构] 中依赖于验证发生年份的验证器的例子。
  * {ref "functor-applicative-monad/applicative.md#subtypes"}[子类型] 中包含引用特定值的命题。
  * 几乎所有有趣的命题都是包含值的类型，因此是依值类型，包括判断 {ref "props-proofs-indexing.md"}[数组索引表示] 的有效性的命题，。
