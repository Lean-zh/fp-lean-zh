...
在局部定义之后，局部定义可用的表达式（称为 {kw}`let` 表达式的 __主体__）必须在新行上，从文件中小于或等于 {kw}`let` 关键字列的位置开始。
...
这意味着即使你得到了一个类型，也不能保证它是给定应用的 __正确__ 类型：
...
这种匿名函数表达式的风格通常被称为 __lambda 表达式__，因为在编程语言的数学描述中使用的典型记法使用希腊字母 λ（lambda），而 Lean 有关键字 {kw}`fun`。
...
Lean 中的每个名称都出现在一个 __命名空间__ 中，这是一个名称集合。
...
命名空间可以被 __打开__，这允许其中的名称在不明确限定的情况下使用。
...
在 Lean 中，用 {kw}`s!` 作为字符串前缀会触发 __插值__，其中字符串内花括号中包含的表达式被替换为它们的值。
...
Lean 编译器维护一个表，描述如何将各种类型的值转换为字符串，消息 {lit}`failed to synthesize instance` 意味着 Lean 编译器没有在此表中找到给定类型的条目。
这使用了与在{ref "structures"}[结构章节]中描述的 {anchorTerm Point}`deriving Repr` 语法相同的语言特性。
